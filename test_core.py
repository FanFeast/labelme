...
schema_module = load_module(
    'labelme.schema_manager',
    os.path.join(base_dir, 'labelme', 'schema_manager.py')
)
...
def test_schema_manager():
    ...
    # Test validation
    assert schema.validate_parent_child("box", "face") == True
    assert schema.validate_parent_child("face", "box") == False
    print(f"  Validation: OK")

    # Test parent requirements
    assert schema.requires_parent("box") == False
    assert schema.requires_parent("face") == True
    print(f"  Parent requirements: OK")
...
def test_label_file(collection):
    ...
    try:
        ...
        print(f"  Hierarchy preserved: OK")
...
def main():
    ...
    try:
        schema = test_schema_manager()
        shape = test_hierarchical_shape()
